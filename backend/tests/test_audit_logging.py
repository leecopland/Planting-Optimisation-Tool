...
async def test_audit_log_on_user_creation(
    ...
    result = await async_session.execute(select(AuditLog).filter(AuditLog.event_type == "user_create").filter(AuditLog.details.contains("audituser@example.com")).order_by(AuditLog.timestamp.desc()))
    ...
    assert log_entry.user_id == test_admin_user.id, f"Expected user_id {test_admin_user.id}, got {log_entry.user_id}"
