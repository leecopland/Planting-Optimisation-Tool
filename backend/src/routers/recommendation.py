...
async def get_farm_recs(
    ...
    farms = await get_farm_by_id(db, [farm_id], current_user.id, user_role=current_user.role)
...
async def get_batch_recs(
    ...
    Requires OFFICER role or higher.
    """
    farms = await get_farm_by_id(db, farm_ids, current_user.id, user_role=current_user.role)
